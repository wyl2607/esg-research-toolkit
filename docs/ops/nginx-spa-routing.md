...
- Unknown application paths render the SPA 404 page with a real 404 status; missing asset-like paths return a real 404.
...
The check verifies the root shell, `/companies`, an unknown route returning the SPA 404 page, a missing JavaScript asset, UTF-8 HTML, no-cache HTML, and immutable hashed assets. It requires a deployed build and does not replace a VPS-level DNS or TLS check.
