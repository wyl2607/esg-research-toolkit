...
# 检查是否有 API key 在代码中
if git diff --cached | grep -iE "(api[_-]?key|secret[_-]?key|openai[_-]?key|anthropic[_-]?key)" | grep -v "OPENAI_API_KEY" | grep -v "\.example" > /dev/null 2>&1; then
  echo "❌ 发现可能的 API key 硬编码"
  git diff --cached | grep -iE "(api[_-]?key|secret[_-]?key)" | head -5
  FOUND_SENSITIVE=1
fi
