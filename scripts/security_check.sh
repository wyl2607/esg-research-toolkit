...
# 检查是否有内网 IP 地址（仅检测新增行，避免误报版本号如 0.10.0）
if git diff --cached \
  | grep -E '^\+' | grep -vE '^\+\+\+' \
  | grep -E "(192\.168\.[0-9]{1,3}\.[0-9]{1,3}|10\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}|172\.(1[6-9]|2[0-9]|3[01])\.[0-9]{1,3}\.[0-9]{1,3})" > /dev/null 2>&1; then
  echo "⚠️  发现内网 IP 地址，请确认是否需要脱敏"
  git diff --cached \
    | grep -E '^\+' | grep -vE '^\+\+\+' \
    | grep -E "(192\.168\.[0-9]{1,3}\.[0-9]{1,3}|10\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}|172\.(1[6-9]|2[0-9]|3[01])\.[0-9]{1,3}\.[0-9]{1,3})" | head -3
fi
