...
const ROWS: RowDef[] = [
  ['Taxonomy %', (c) => c.taxonomy_aligned_revenue_pct != null ? `${c.taxonomy_aligned_revenue_pct.toFixed(1)}%` : '—'],
  ['Scope 1 (t)', (c) => c.scope1_co2e_tonnes?.toLocaleString() ?? '—'],
  ['Renewable %', (c) => c.renewable_energy_pct != null ? `${c.renewable_energy_pct.toFixed(1)}%` : '—'],
  ['Employees', (c) => c.total_employees?.toLocaleString() ?? '—'],
  ['Female %', (c) => c.female_pct != null ? `${c.female_pct.toFixed(1)}%` : '—'],
        ...
        <div className="space-y-8">
          ...
          <div>
            <h2 className="text-lg font-semibold mb-4">
              Taxonomy Radar (use Taxonomy page to load scores first)
            </h2>
            <p className="text-sm text-slate-400">
              Radar charts are displayed on the Taxonomy page after running a
